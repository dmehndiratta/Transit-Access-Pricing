...
with tab_prem:
    ...
    if not results:
        ...
        st.markdown(
            "The naive effect mostly reflects that central locations are both "
            "transit-rich and expensive. Once distance-to-core is controlled "
            "for, the standalone transit premium attenuates sharply — the "
            "honest estimate is the controlled one.")
        st.divider()
...
with tab_method:
    st.markdown(
        ...
        "amenity-rich; without a centrality control the transit effect is "
        "confounded. The headline result is the attenuation once "
        "`dist_to_core_m` is added.\n\n"
        "**Interpretation.** Airbnb nightly prices reflect short-term/tourist "
        "willingness to pay, not commuter rent — a related but distinct "
        "premium.\n\n"
        "**Vintage.** Built on a recent snapshot; Montreal's 2025 short-term-"
        "rental rules reshaped the market, so listing counts are lower than in "
        "pre-2025 data.\n\n"
        "**Data.** Inside Airbnb (CC BY 4.0); STM GTFS (attribution required).")